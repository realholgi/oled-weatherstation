...
void Adafruit_SSD1305::drawPixel(int16_t x, int16_t y, uint16_t color)
{
  ...
  //  buffer[x+ (y/8)*SSD1305_LCDWIDTH] &= ~_BV((y%8));
  switch (color) {
    case WHITE:   buffer[x + (y / 8)*SSD1305_LCDWIDTH] |=  (1 << (y & 7)); break;
    case BLACK:   buffer[x + (y / 8)*SSD1305_LCDWIDTH] &= ~(1 << (y & 7)); break;
      //case INVERSE: buffer[x+ (y/8)*SSD1305_LCDWIDTH] ^=  (1 << (y&7)); break;
  }
}
...
void Adafruit_SSD1305::begin(uint8_t i2caddr)
{
  ...
  command(SSD1305_SETSTARTLINE); // line #0
  command(0x00);
  //command(0x2E); //??
  ...
  command(0x02); //     0=Hori, 1=Vert, 2=Page
  ...
  command(0xF1);
  command(SSD1305_SETCOMPINS);                    // 0xDA
  command(0x12);  // disable COM left/right remap
  command(SSD1305_SETLUT);
  command(0x3F);
  command(0x3F);
  command(0x3F);
