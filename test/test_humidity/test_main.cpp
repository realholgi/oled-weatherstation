...
void test_absolute_humidity_known_room_value(void) {
    TEST_ASSERT_FLOAT_WITHIN(0.05f, 8.64f, berechneTT(20.0, 50.0));
}

void test_dewpoint_known_room_value(void) {
    TEST_ASSERT_FLOAT_WITHIN(0.05f, 9.26f, RHtoDP(20.0, 50.0));
}

void test_absolute_humidity_increases_with_relative_humidity(void) {
    TEST_ASSERT_GREATER_THAN(berechneTT(20.0, 40.0), berechneTT(20.0, 60.0));
}

void test_dewpoint_increases_with_temperature_at_same_relative_humidity(void) {
    TEST_ASSERT_GREATER_THAN(RHtoDP(10.0, 50.0), RHtoDP(20.0, 50.0));
}
